...
The claim command uses one PostgreSQL `INSERT ... ON CONFLICT DO UPDATE` statement. It admits a new batch, reacquires a `FAILED` batch, or reclaims a `PROCESSING` batch whose renewable fifteen-minute lease expired; a completed or active batch remains a duplicate. Every acquisition receives a new UUID fencing token. The processor renews the lease before each candidate and before completion, while JPA completion and failure updates are conditional on the same token. A previous claimant therefore cannot overwrite a newer attempt after reclamation. Read-side inspection is exposed through a jOOQ query adapter, keeping query construction out of the command adapter. Because recovery can repeat downstream calls, every order, execution, and settlement port is required to be idempotent by its stable source identifier.
...
- A processing claim renews its lease during progress; an abandoned lease becomes eligible for atomic reclamation after fifteen minutes.
- A reclaimed batch rejects heartbeat, completion, and failure updates from its previous fencing token.
